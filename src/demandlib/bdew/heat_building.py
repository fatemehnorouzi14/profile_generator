...
from src.demandlib.tools import add_weekdays2df
...
class HeatBuilding:
    ...
    def get_normalized_bdew_profile(self):
        ...
        kw = 1.0 / (sum(h * f) / 24)
        heat_profile_normalized = kw * h * f * self.sf

        return heat_profile_normalized

    def hot_water_demand(self, add_weight=False ):
        """Calculation of the hourly hot water demand"""
        self.df["temperature"] = self.temperature.values
        self.df["temperature_geo"] = self.weighted_temperature(
            how="geometric_series"
        )
        ...
        kw = 1.0 / (sum(h * f) / 24)
        if add_weight:
            self.weight = self.extend_sf_data()
            hot_water_demand = sf * self.annual_heat_demand * kw * f * d * self.weight
            hot_water_demand =pd.DataFrame(hot_water_demand)
        else:
            hot_water_demand = sf * self.annual_heat_demand * kw * f * d
            hot_water_demand =pd.Series(hot_water_demand, name='temperature_geo', index=self.df.index)

        return hot_water_demand

    def space_heating_demand(self):
        ...
        h = a / (1 + (b / (self.df["temperature_geo"] - 40)) ** c) + d
        kw = 1.0 / (sum(h * f) / 24)
        space_heating_demand =  (a / (1 + (b / (self.df["temperature_geo"] - 40)) ** c)) * f * self.sf * kw * self.annual_heat_demand

        return space_heating_demand


    def extend_sf_data(self):
        # Assuming there are 365 days in a year
        days_per_week = 7
        hours_per_day = 24
        weeks_per_year = 365 // days_per_week
        custom_data = self.data_dict
        # Create an empty list to hold the extended series
        extended_series = []

        # Extend the data for each day of the week
        for _ in range(weeks_per_year):
            for day in ["Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday"]:
                extended_series.extend(custom_data[day][0])  # Assuming the data for each day is in the first row

        # If there are any additional hours left to reach 8760, repeat the last week's data
        remaining_hours = 8760 - len(extended_series)
        if remaining_hours > 0:
            last_week_data = custom_data["Sunday"][0]  # Assuming the data for Sunday is in the first row
            extended_series.extend(last_week_data * (remaining_hours // hours_per_day))
            extended_series.extend(last_week_data[:remaining_hours % hours_per_day])

        sf = pd.Series(extended_series, name='temperature_geo')
        return sf
